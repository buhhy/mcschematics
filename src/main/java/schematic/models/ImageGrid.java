...
public class ImageGrid extends JComponent implements Printable, MouseListener {
	...
	private static Dimension imgDim = ImageProvider.DEFAULT_IMAGE_SIZE;
	 ...
	 */
	public ImageGrid(Slice slice, float zoom) {
	 ...
	 */
	public ImageGrid(Slice slice, float zoom, Color gridColor, Color markColor) {
		this.slice = slice;
	...
	public int print(Graphics g, PageFormat pageFormat, int pageIndex) throws PrinterException {
		...
		// 72 dpi
		float prevZoom = this.getZoom();
		this.setZoom(1.0f); // max precision
		double width = pageFormat.getImageableWidth();
		double height = pageFormat.getImageableHeight();
		float hZoom = (float) width / this.getPreferredSize().width;
		float vZoom = (float) height / this.getPreferredSize().height;
		float zoom = (hZoom < vZoom) ? hZoom : vZoom;
		g2d.scale(zoom, zoom);
