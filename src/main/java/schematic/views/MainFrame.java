package schematic.views;

import schematic.models.ImageGridStack;
import schematic.models.images.ImageProvider;
import schematic.models.SchematicReader;
import schematic.models.SliceStack;
...
public class MainFrame extends JFrame implements ActionListener, ChangeListener {
    private static final int DEFAULT_ZOOM = 2 * 10;
    ...
    DragScrollListener dragScrollListener;
    ...
    public MainFrame() {
        ...
        this.pnlGrid.setBackground(Color.WHITE);

        // init buttons
        this.btnRotateCW = new JButton(new ImageIcon(ClassLoader.getSystemResource(ImageProvider.MISC_IMAGES_DIRECTORY + "CW.gif")));
        this.btnRotateCW.setActionCommand("RCW");
        this.btnRotateCW.addActionListener(this);
        this.btnRotateCCW = new JButton(new ImageIcon(ClassLoader.getSystemResource(ImageProvider.MISC_IMAGES_DIRECTORY + "CCW.gif")));
        ...
        this.sldZoom.setValue(DEFAULT_ZOOM);
        ...
        for (int i = 0; i <= 100; i += 10) {
            zoomLabelTable.put(i, new JLabel(Integer.toString(i / 10)));
        }
        ...
        this.sldLayer.setMinorTickSpacing(1);
        ...
        this.dragScrollListener = new DragScrollListener(this.scrGrid, this.pnlGrid);
        Toolkit.getDefaultToolkit().addAWTEventListener(this.dragScrollListener, eventMask);
    ...
    private void openFile(File file) {
        try {
            ...
            this.stack.trim();
            float zoom = (this.sldZoom.getValue() / 10.0f);
            ...
            if (SchematicReader.hasErrorHappened()) {
                JOptionPane.showMessageDialog(
                        null, "There were some faulty blocks in the schematic. They were replaced with air.",
            ...
            this.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
            JOptionPane.showMessageDialog(
                    null, "Classic file format is not supported", "Classic not supported", JOptionPane.ERROR_MESSAGE);
            return;
            ...
            this.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
            JOptionPane.showMessageDialog(
                    null, "Ran out of memory while trying to open schematic",
                    "Out of Memory", JOptionPane.ERROR_MESSAGE);
            return;
        }

        enableSchematicControls(true);
        if (this.currentLayer >= this.images.getStackSize())
            this.currentLayer = this.images.getStackSize() - 1;
        this.sldLayer.setMaximum(this.images.getStackSize());
        if (this.images.getStackSize() == 1)
            this.sldLayer.setEnabled(false);
    ...
    private void displayError(Exception e) {
        JOptionPane.showMessageDialog(
                null, "Error: " + e.getMessage(), e.getClass().getSimpleName(), JOptionPane.ERROR_MESSAGE);
        e.printStackTrace();
        System.exit(1);
    }
    ...
    private void setupShortcutScrollListeners() {
        this.scrGrid.addMouseWheelListener(new MouseWheelListener() {
            @Override
            public void mouseWheelMoved(MouseWheelEvent event) {
                int alt = event.getModifiersEx() & MouseWheelEvent.ALT_DOWN_MASK;
                int ctrl = event.getModifiersEx() & MouseWheelEvent.CTRL_DOWN_MASK;

                scrGrid.setWheelScrollingEnabled(alt == 0 && ctrl == 0);
            }
        });
        this.addMouseWheelListener(new MouseWheelListener() {
            ...
            public void mouseWheelMoved(MouseWheelEvent event) {
                ...
                // alt-scroll to change layers
                if (alt != 0 && ctrl == 0) {
                    if (sldLayer.isEnabled()) {
                        sldLayer.setValue(
                                sldLayer.getValue() - sldLayer.getMinorTickSpacing() * event.getWheelRotation());
                    }
                }
                // ctrl-scroll to change zoom
                else if (alt == 0 && ctrl != 0) {
                    if (sldZoom.isEnabled()) {
                        sldZoom.setValue(
                                sldZoom.getValue() - sldZoom.getMinorTickSpacing() * event.getWheelRotation());
                    }
                }
            }
        });
    ...
    public void actionPerformed(ActionEvent arg0) {
        if (arg0.getActionCommand().equals("OPEN")) {
            ...
            if (fc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
                ...
                if (!f.isDirectory()) {
                    if (f.exists()) {
                        int reply = JOptionPane.showConfirmDialog(this, "File allready exists. Overwrite?", "File allready exists", JOptionPane.YES_NO_OPTION);
                    ...
                    try {
                        ...
                        try {
                            ...
                            JOptionPane.showMessageDialog(this, "Error while saving image " + f.getName() + ":\n"
            ...
            new ExportDialog(this, stack, images, openedFile, pnlGrid.getBackground(), currentZoom, false);
        } else if (arg0.getActionCommand().equals("EXPGIF")) {
            new ExportDialog(this, stack, images, openedFile, pnlGrid.getBackground(), currentZoom, true);
            ...
            if (fc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
                ...
                if (!f.isDirectory()) {
                    ...
                    try {
                        ...
                        e.printStackTrace();
                        JOptionPane.showMessageDialog(this, "Error while saving text file " + f.getName() + ":\n"
                                + e.getLocalizedMessage(), "Could not save", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
 ...
 * @author Taiko
 */
class DragScrollListener implements AWTEventListener {
    ...
    public DragScrollListener(JScrollPane scrPane, JComponent comp) {
        this.comp = comp;
